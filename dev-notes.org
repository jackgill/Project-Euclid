...
** aliases for building names
** badge for number of requests
** activity summary on home page
* Someday/maybe
...
** TODO need to extract date parsing and error handling logic in listings and request controllers
...
** TODO for the 'new listing' form, if there is a validation error, the error page will incorrectly display the building and spot selects, since they are only hidden if the values are present on the query string. this needs to be fixed, and additionally there should be a server-side check that the user can only create listings for spots they own.
** why again does the 'view requests' page not show start and end dates?
** TODO better error message when you try to fulfill your own request
** Notifications system
...
** TODO request editing is broken -- spurious validation errors, "Show | Back" links should be removed
** TODO add html5 shim?
** TODO use bootstrap modal for confirmation dialogs?
** TODO "about us" in footer
** TODO dashboard should suggest actions
** scratch
