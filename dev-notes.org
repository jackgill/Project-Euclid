...
- should check that two users don't create the same spot
- need a rake task to expire old listings / availability (w/ notifications)

require_owner_or_admin (global, whitelist public pages)
only for non-GET verbs? nah - transactions should be GET-protected

Release Notes:
- "Jump" link has been moved from page header to admin tool
- Users must now actually be admins in order to see the "Admin" link in the header and view the admin page
- TODO: verify admin accounts in prod
