...
** TODO separate password change page?
...
** scratch
need to get the db schema right for users / user_preferences
need to add building_id, notify_new_request, notify_new_listing to users table
need to update seed data to reflect new structure
need appropriate "not null" and default value constraints for preferences columns
need to expose to end user a form for editing user data

create a migration to
1) drop prefs table
2) add prefs columns to users table
3) get constraints right for prefs columns
