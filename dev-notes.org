...
I should probably get at least the default unit tests to pass. Not saying that I'm going to go crazy w/ the unit testing, but testing is one of the major benefits of the rails framework and I'm basically denying myself that benefit by letting all the default tests fail.


so the tests ignore seed data and only look at fixtures
rake db:test:load loads the fixtures into the test database. I think.
rake db:fixtures:load loads the fixture data into the dev database, which is *not* what we want
for some reason, the transaction fixture isn't pointing to a real spot
something about the load order of the fixtures breakin foreign key constraints?

WELL
it seems that to reference a foreign key, you should use the fixture NAME, not the ID
JESUS
