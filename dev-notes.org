...
Release Notes
- Added date range validation for listings, requests, and transactions. Date range validation ensures that:
1) The start date is not in the past
2) The end date is greater than the start date
- Removed "taken" column from listings table, since we are now implementing this functionality using the availability table.
