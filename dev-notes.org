...
we don't seem to have a process for moving completed transactions out of the table.

so we can't delete a listing which has a cancelled transaction associated with it. hrm...
what is the expected behavior? should we delete the cancelled transaction?
should cancelled transactions be moved to a different table, w/o the fk constraint?
or should we make listings "cancellable" as well? After all, this problem arises from the fact that listings are deleted while transactions are simply marked cancelled
